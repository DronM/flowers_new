...
<h3>Комплектация №<xsl:value-of select="row/number"/> от <xsl:value-of select="row/date_time_descr"/></h3>
<div>
	<p>
		<span>Салон:</span>
	...
	<p>
		<span>Букет:</span>
		<span>код:<xsl:value-of select="row/product_id"/>,<xsl:value-of select="row/product_descr"/></span>
	</p>		
	<p>
