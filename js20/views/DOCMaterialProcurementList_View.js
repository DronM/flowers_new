function DOCMaterialProcurementList_View(id,options){	
	...
	var model = new DOCMaterialProcurementList_Model({"data":options.modelDataStr});
	...
	filters.supplier = {
		"binding":new CommandBinding({
			"control":new SupplierEditRef(id+":filter-ctrl-supplier",{
				"app":options.app,
				"keyIds":["supplier_id"],
				"app":options.app
				}),
	...
	grid_rows.push(	
		new GridCellHead(id+":grid:head:supplier_descr",{
			"columns":[
				new GridColumn("supplier",{"field":model.getField("supplier_descr")})
			],
			"sortable":true
		})
	);
	...
	this.addElement(new GridAjx(id+":grid",{
		...
		"keyIds":["id"],
		"editInline":false,
		"editWinClass":DOCMaterialProcurement_Form,
		...
		"commands":new GridCmdContainerDOC(id+":grid:cmd",{
			...
			"popUpMenu":popup_menu,
			"colTemplate":"DOCMaterialProcurementList",			
			"filters":filters,
			"cmdDOCUnprocess":true,
			"cmdColManager":false,
			"variantStorageModel":options.variantStorageModel,
			"app":options.app
		}),
